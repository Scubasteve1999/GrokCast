...
**Today:** Storm-first feed (`FeedItem.defaultOrder`): Now → one official NWS chip (if live) → Hourly curve → Outlook radar plate → Your News → Conditions (elevated AQI / AQ alert / low vis only) → Daily → Nearby (fire only). First viewport on iPhone 16 must include a Your News card, not a header peek. `WeatherStage` photography under type. Weather modules use `weatherModuleStyle()` (material on the stage); Settings / Paywall stay solid `cardStyle`. Error banner above Now. Take / Imagine / Next-hour strip are not feed cards (Imagine is off Today and Sky Check). Future pill on the Outlook plate is Yearly-only. Outlook hoists Site Doppler only for precip now / next-hour precip / radar-relevant warning-watch; heat and air-quality advisories stay National MapsGL (and still show the Alerts chip).
