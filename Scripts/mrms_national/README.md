...
Serves `http://127.0.0.1:8765/timestamps.json` and `{id}/{z}/{x}/{y}.png` (z3–z8). Tiles are opaque-blurred on the native ~1 km dBZ grid, bilinear-sampled, then LUT-snapped (`PAINT_VERSION` 5) — no overlay pyramid. Simulator Debug builds use that URL. Device/TestFlight has no localhost — MapsGL rain is the fallback until a CDN base URL is set on `MRMSRadarService`.
