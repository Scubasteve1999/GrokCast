...
enum EntitlementChecker {
  ...
  /// Widget AI one-liner is generated by Grok. Pro alone is not enough until the
  /// hosted proxy is live — same rule as `canUseGrokAI`.
  static func canUseWidgetGrokBrief(
    subscription: SubscriptionManager,
    hasDeveloperKey: Bool
  ) -> Bool {
    guard subscription.isPro else { return false }
    if hasDeveloperKey { return true }
    return GrokProxyConfiguration.isConfigured
  }
