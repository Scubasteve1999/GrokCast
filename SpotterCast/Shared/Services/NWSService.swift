...
final class NWSService {
  ...
  private let userAgent = "SpotterCast/1.0 (https://scubasteve1999.github.io/GrokCast)"
  ...
  // Returns mapped to existing SpotterCastWeather (no struct changes, Date ids preserved)
  ...
  func fetchForecast(for location: SavedLocation) async throws -> SpotterCastWeather {
    try Task.checkCancellation()
